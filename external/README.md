...
| `DCNet/` | 张量网络传输框架 | **不启用（V9，2026-09-07）**：数据面走对象存储总线（根 DESIGN §9），`BUILD_DCNET=OFF`；P3 直连需求经 D13 提案机制反馈 |
| 引擎适配器 | Builtin / OnnxRuntime / OpenAI（`BUILD_ENGINES` 开关，默认 OFF） | `client`（OnnxRuntime，V10）、`server/sidecar`（视本地节点引擎需求） |
...
C++ 构建经 vcpkg（见 `client/vcpkg.json`）；开关：`BUILD_ENGINES=ON`（client 用 OnnxRuntime 适配器）/ `BUILD_DCNET=OFF`（V9）。仅需要核心源码可用 git sparse-checkout（`DCinfer DCIr cmake vcpkg.json CMakeLists.txt`）。
